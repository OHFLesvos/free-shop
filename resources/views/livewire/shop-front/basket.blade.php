                        ...
                        @foreach($this->products->whereIn('id', $basket->items()->keys()) as $product)
                            <tr>
                                ...
                                <td class="align-middle pe-3 fit">
                                    ...
                                    <button
                                        type="button"
                                        class="btn btn-sm @unless($basket->get($product->id) < $product->quantityAvailableForCustomer() && $product->price <= $this->availableCredit) btn-secondary @else btn-success @endunless"
                                        wire:click="add({{ $product->id }}, 1)"
                                        wire:loading.attr="disabled"
